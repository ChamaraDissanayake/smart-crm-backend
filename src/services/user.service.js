...
const register = async (userData) => {
    const userId = uuidv4();
    await userModel.create({ id: userId, ...userData });


    const user = await userModel.findById(userId);
    const token = jwtService.generateJWT({ type: 'verify-email', userId: user.id, name: user.name, email: user.email, isVerified: false }, '24h');
...
const login = async (email, password) => {
    ...
    return jwtService.generateJWT({ type: 'login', userId: user.id, name: user.name, email, isVerified: user.is_verified }, '7d');
};
...
const requestPasswordReset = async (email) => {
    try {
        ...
        const token = jwtService.generateJWT({ type: 'password-reset', userId: user.id, name: user.name, email, isVerified: user.is_verified }, '15m');
...
const resendVerificationEmail = async (email) => {
    ...
    const token = jwtService.generateJWT({ type: 're-verify-email', userId: user.id, name: user.name, email: user.email, isVerified: user.is_verified }, '24h');
